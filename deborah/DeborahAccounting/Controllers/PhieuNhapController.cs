...
namespace DeborahAccounting.Controllers
{
    ...
    public class PhieuNhapController : Controller
    {
        ...
        public PhieuNhapController(AppDbContext db) => _db = db;

        public async Task<IActionResult> Index()
        {
            var list = await _db.PhieuNhapXuat
                .Where(p => p.SoCT.StartsWith("PN"))
            ...
            ViewBag.DSNCC = await _db.DMNCC.ToListAsync();
        ...
        public async Task<IActionResult> Create([FromBody] PhieuNhapDto dto)
        {
            ...
            var phieu = new PhieuNhapXuat
            {
                ...
                MaNCC = dto.MaNCC ?? "", MaSP = dto.MaSP ?? "",
                CTLQ = dto.CTLQ, LyDo = dto.LyDo
            };
        ...
        public async Task<IActionResult> Edit([FromBody] PhieuNhapDto dto)
        {
            ...
            phieu.MaNCC = dto.MaNCC ?? ""; phieu.MaSP = dto.MaSP ?? "";
            phieu.CTLQ = dto.CTLQ; phieu.LyDo = dto.LyDo;
        ...
        public async Task<IActionResult> Delete([FromBody] string id)
        {
            ...
            if (phieu == null) return Json(new { success = false, message = "Không tìm thấy" });
            var ct = await _db.CTPHIEUNX.Where(c => c.SoCT == id).ToListAsync();
            _db.CTPHIEUNX.RemoveRange(ct);
        ...
        public async Task<IActionResult> Print(string id)
        {
            var phieu = await _db.PhieuNhapXuat
                .Include(p => p.NhaCungCap).Include(p => p.Kho)
    ...
    public class PhieuNhapDto
    {
        ...
        public List<CTPhieuNXDto>? ChiTiet { get; set; }
    }

    public class CTPhieuNXDto
    {
